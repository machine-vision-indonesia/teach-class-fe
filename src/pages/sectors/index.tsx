...
export default function ManageSectorsPage() {
      ...
      <main>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          <div style={{ flexGrow: 1 }}>
            <Typography variant='h2'>Manage Sector</Typography>
            <Breadcrumbs
          ...
          >
            <MenuItem
              ...
              href='/sectors/add'
            >
              <span style={{ fontWeight: 500, fontSize: '14px', letterSpacing: '0.43px' }}>Data Sector</span>
              <Icon fontSize='16px' icon='material-symbols:add' />
            </MenuItem>
            <MenuItem
              style={{ padding: '8px 16px', margin: 0, justifyContent: 'space-between' }}
              component={NextLink}
              href='/sectors/add/bulk'
            >
              <span style={{ fontWeight: 500, fontSize: '14px', letterSpacing: '0.43px' }}>Bulk Data Sector</span>
        ...
        >
          ...
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }}>
              ...
              <TableBody>
                {sectors.map(sector => (
                  <TableRow key={sector.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    <TableCell style={{ color: theme.palette.text.primary }}>{sector.code}</TableCell>
                    <TableCell style={{ color: theme.palette.text.primary }}>{sector.name}</TableCell>
                    <TableCell style={{ color: theme.palette.text.primary }}>{sector.plant}</TableCell>
                        ...
                        onClick={async () => {
                          try {
                            const qrValue = await QRCode.toDataURL(
                              `${env('NEXT_PUBLIC_BASE_URL')}/sectors/${sector.id}`,
                              {
                                width: 256
                              }
                            )
