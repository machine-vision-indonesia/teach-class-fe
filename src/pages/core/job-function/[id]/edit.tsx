...
import { useGetJobFunction } from 'src/components/complexes/job-function/service/edit/useGetJobFunction'
...
import { isEditSuccessAtom } from '../../../../components/complexes/job-function/atoms'
...
export default function Page() {
  ...
  const jobFunction = useGetJobFunction({ id: router.query.id as string, enabled: router.isReady })
  ...
  const [isEditJFFailed, setIsEditJFFailed] = useState(false)
  ...
  const [, setIsEditSuccess] = useAtom(isEditSuccessAtom)
  ...
  const updateJobFunctionMutation = useMutation({
    async mutationFn(data: Schema) {
      ...
      if (response.data.data.length && response.data.data.at(0).id != (router.query.id as string)) {
        throw new Error('Code already exists')
      }

      return client.api.patch(`/items/mt_job_functions/${router.query.id as string}`, {
        description: data.description,
        code: data.code,
        name: data.name,
        sto: data.department.id,
        job_level: data.jobLevel?.id ?? null
      })
    }
  })
  ...
  async function onAddSubmit() {
    ...
    try {
      await updateJobFunctionMutation.mutateAsync(data)
      await queryClient.invalidateQueries()
      router.push('/core/job-function')
      setIsEditSuccess(true)
      updateJobFunctionMutation.reset()
    } catch (e) {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setIsEditJFFailed(false)
    }, 4000)
  }, [isEditJFFailed])

  useEffect(() => {
    if (jobFunction.isSuccess) {
      if (jobFunction.data.data.sto != null) {
        form.reset({
          ...
          department: {
            id: jobFunction.data.data.sto?.id,
            label: jobFunction.data.data.sto?.name
          },
    ...
    form,
    jobFunction.data?.data.code,
    jobFunction.data?.data.description,
    jobFunction.data?.data.job_level.id,
    jobFunction.data?.data.job_level.name,
    jobFunction.data?.data.name,
    jobFunction.data?.data.sto,
  ...
  return (
    <>
      <main>
        <Typography variant='h2'>Edit Job Function</Typography>
        <Breadcrumbs
            ...
            >
              ...
              >
                <div style={{ gridColumn: 'span 2' }}>
                  ...
                  >
                    Complete the data to edit a Job Function
                  </Typography>
                </div>
                ...
                <div>
                  ...
                  {form.formState.errors.code ? (
                    <FormHelperText sx={{ color: 'error.main' }}>
                      job function {form.formState.errors.code.message}
                    </FormHelperText>
                  ) : null}
                </div>
                ...
                <div>
                  ...
                  {form.formState.errors.name ? (
                    <FormHelperText sx={{ color: 'error.main' }}>
                      job function {form.formState.errors.name.message}
                    </FormHelperText>
                  ) : null}
                </div>
                  ...
                  <Controller
                    ...
                    render={({ field: { value, onChange, ...rest }, fieldState }) => (
                      <div style={{ gridColumn: 'span 3' }}>
                        ...
                        {fieldState.invalid ? (
                          <FormHelperText sx={{ color: 'error.main' }}>job level is required field</FormHelperText>
                        ) : null}
                      </div>
                    )}
                  />
      ...
      <Modal
        ...
        variant='warning'
        loading={updateJobFunctionMutation.isLoading || updateJobFunctionMutation.isSuccess}
        positiveLabel='Yes'
      ...
      {isEditJFFailed ? (
        <Box position='fixed' top='85px' right='24px'>
          <Alert
            variant='contained'
            content='Your job function was failed to edited'
            color='error'
            title='Edit Failed'
            icon='ic:baseline-do-disturb'
            onClose={() => setIsEditJFFailed(false)}
          />
        </Box>
      ) : null}
    </>
